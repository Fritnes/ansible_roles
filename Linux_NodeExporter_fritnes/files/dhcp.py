...
for lease in report_dataset:
        print('dhcpd_active_leases{ip="' + format(lease['ip_address'], '<15') +  '",' + \
                'mac="' + format(lease['hardware'], '<17') + '",' + \
                'time="' + format(str((lease['ends'] - now) if lease['ends'] != 'never' else 'never'), '>20').replace(" ", "") + '",' + \
                'ddns="' + lease['client-hostname'] + '"} 1')
